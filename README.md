...
**Registry consensus fallback deleted (2026-08-09)**: `resolve_consensus()` used to fall back to the hand-frozen `consensus` block in `config/research_stocks.json` whenever the automated fetch came back empty. Those frozen values had not moved since each stock was registered (中际旭创 recorded 480亿 against an actual 548亿, 14% off), so a failed fetch produced neither an error nor a blank but a **calmly rendered, wrong PE** — and under the "read the report, form a judgement" usage that is considerably more dangerous than a visible gap. At the user's direction the fallback is gone: a failed fetch yields `consensus_source="missing"` with an empty denominator, the page renders no multiple, and the health check alerts on the absence. `compare_with_registry()`, `significant_changes()` and the `deltas_vs_registry` field went with it — once the frozen values are gone their input is permanently empty, and leaving them in place would suggest a manual review gate that no longer exists. Every stock's `consensus` block was removed from the registry, and a test asserts that a leftover block is ignored even if one reappears. The health alert's wording changed with the failure mode: from "the denominator fell back to a stale registry value" to "the denominator is missing" — the harm shifted from showing a wrong number to showing one fewer number, but silence would still leave the page quietly incomplete.

**Thin-coverage threshold removed (2026-08-09)**: `MIN_MEDIAN_SAMPLES = 5` withheld the median and set `insufficient_samples` whenever fewer than five brokers covered a year, and the page rendered that as a red "样本不足" warning. The user's ruling: some companies simply are covered by few brokers, and that is a true state of the world rather than a data defect. Both the constant and the flag are gone; the median is computed regardless (with two samples it equals the mean, which is harmless and honest), the preferred statistic no longer switches on sample count (trimming degenerates to the arithmetic mean on small samples anyway, so the special case bought nothing but an inconsistent label downstream), and the broker count remains on the page as a neutral fact. 长鑫科技 now reads "覆盖机构 4 家（2 家有逐家明细）" with no warning colour — while its ⚠ badge remains, driven by the 25.4%/7.3% range overlap between the two sources, which is a genuine data-quality problem independent of how many brokers there are.

**Buy-point price alerts deleted (2026-08-09)**: `research_price_alert.py`, its test module, the `price_alerts` blocks in the registry and the (paused since 2026-06-04) crontab entry were all removed at the user's direction — "not in use now, may be rewritten later". Git history retains the implementation.
...
**Research page「相关新闻」section, wired (2026-08-11)**: `scripts/update_research_news.py` refreshes a per-stock news/announcements panel consumed by the 11 research pages. **Cron**: daily 17:00 BJT (`0 9 * * *` UTC, every day — announcements often land on weekends) via `cron-wrapper --name research-news --timeout 900 --lock`, chained to `commit_research_data.sh "data/*-news.json" "data/*-news-raw.jsonl"`.

*Source split.* Announcements are the backbone (A-share via cninfo/巨潮, HK via HKEX's disclosure-search endpoint/披露易); news is supplementary (A-share via East Money `stock_news_em`, HK via yfinance). The two layers are fetched with deliberately different strategies: the announcement APIs accept a date range, so each run simply re-pulls the trailing 30 days and carries no history burden. The news API does not — East Money's `stock_news_em` returns a fixed 10 most-recent items regardless of window, and 盛科通信's 10 items measured on 2026-08-11 covered only 4 calendar days. A 30-day window is therefore unreachable from a single call; each run appends any new-by-`url` items to `{key}-news-raw.jsonl` and reads the trailing 30 days back out of that file. The jsonl is the one artifact in this pipeline that cannot be reconstructed after the fact (the source only ever shows 10), so it is written via a temp-file + `os.replace()` swap and never truncated by the window.

*Six-layer sort.* `重要事项 → 公司公告 → 相关新闻 → 程序性文件 → 交易与资金面 → 板块与市场` (`news_rules.LAYER_ORDER`). This is a **sort, not a filter** — nothing is dropped, per the user's direction that trading/capital-flow items belong at the bottom of a fundamentals-first page rather than off the page. `程序性文件` (procedural) and `板块与市场` (sector) are collapsed by default in the page UI.

*Relevance gating.* Only **news** items are subject to a relevance check: a headline that contains none of the company name, stock code, or a configured alias is demoted to `板块与市场` regardless of its original layer. Announcements are exempt — they are the company's own first-party disclosure by construction, so the check would be redundant and could only misfire. Aliases live in `config/research_stocks.json`'s `news_aliases` field per stock; an unconfigured stock still runs the check (against name+code only) but prints a WARN so a silent sector-only collapse is at least visible in the log.

*Empty vs. error — do not conflate.* `announcements_empty=True` means the fetch **succeeded** and genuinely found 0 items in the window; `announcements_error=True` means the fetch **failed** and the true count is unknown. The page must never render "近 30 天无公告披露" for the latter — that would state a fact about the company's disclosure activity that the pipeline never actually observed, and `write_and_deploy()` refuses to overwrite an existing file when `announcements_error` is set, so a transient failure never blanks out yesterday's real data. The tricky part is that akshare's cninfo call raises a bare `KeyError` when a narrow window's result set is empty (it tries to select columns off an empty DataFrame) — indistinguishable at the exception level from "the call is genuinely broken". `fetch_cninfo_announcements` resolves this by re-querying with a 365-day window on `KeyError`: if the wide query also raises, it's a real fault and the narrow `KeyError` is re-raised up to the `None` sentinel path; if the wide query succeeds, the narrow window's 0 result was real and `[]` is returned. Verified live: 长光华芯 genuinely has 0 announcements in its trailing 30 days as of 2026-08-11. **Probe retry (2026-08-27, `2a00b11`)**: the probe is itself one more network call, so a rate-limited second shot mislabels a genuine 0-result as a fault — which is exactly what the 2026-08-26 17:00 BJT cron failure was: cninfo returned non-JSON (`JSONDecodeError: Expecting value: line 1 column 1`) to 688825's probe, the run exited 1, and the next morning the same queries came back clean (30d = 3 rows, 365d = 20 rows, measured). Transient errors — `json.JSONDecodeError`, `TimeoutError`, `OSError` (which covers `requests.exceptions.RequestException`) — now retry once after `PROBE_RETRY_DELAY_S` = 2s, and only a second failure is called a fault. `KeyError` is deliberately **excluded** from that set: a wide-window `KeyError` is the schema-really-changed signal, and treating it as transient would re-commit the silent-zero defect this whole mechanism exists to prevent. `test_probe_keyerror_does_not_retry` pins that exclusion and was mutation-verified (adding `KeyError` to the transient tuple turns it red).

*Acceptance condition.* Run for two weeks, then check whether the user actually opens this section of a research page. Whether that happens — not whether the data pipeline is clean — is what decides if Phase 2 (industry-level sources) gets built.
